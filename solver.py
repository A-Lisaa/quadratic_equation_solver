...
class Equation:
    def __init__(self, equation: str):
        ...
        self.a_left_side = self.calculate_coefficient_same_side(self._coefficients[0][0])
        self.a_right_side = self.calculate_coefficient_same_side(self._coefficients[0][1])
        self.a = self.calculate_coefficient(self._coefficients[0])

        self.b_left_side = self.calculate_coefficient_same_side(self._coefficients[1][0])
        self.b_right_side = self.calculate_coefficient_same_side(self._coefficients[1][1])
        self.b_left_sign = '-' if self.b_left_side < 0 else '+'
        self.b_right_sign = '-' if self.b_right_side < 0 else '+'
        self.b = self.calculate_coefficient(self._coefficients[1])
        self.b_sign = '-' if self.b < 0 else '+'

        self.c_left_side = self.calculate_coefficient_same_side(self._coefficients[2][0])
        self.c_right_side = self.calculate_coefficient_same_side(self._coefficients[2][1])
        self.c_left_sign = '-' if self.c_left_side < 0 else '+'
        self.c_right_sign = '-' if self.c_right_side < 0 else '+'
        ...
        if self.discriminant > 0:
            if self.a_right_side != 0 or self.b_right_side != 0 or self.c_right_side != 0:
                self.return_string = f"""{equation}
{self.a_left_side}{self._variable_name}² {self.b_left_sign} {str(self.b_left_side).replace('-', '') if '-' in str(self.b_left_side) else self.b}{self._variable_name} {self.c_left_sign} {str(self.c_left_side).replace('-', '') if '-' in str(self.c_left_side) else self.c_left_side} = {self.a_right_side}{self._variable_name}² {self.b_right_sign} {str(self.b_right_side).replace('-', '') if '-' in str(self.b_right_side) else self.b_right_side}{self._variable_name} {self.c_right_sign} {str(self.c_right_side).replace('-', '') if '-' in str(self.c_right_side) else self.c_right_side}
{self.a}{self._variable_name}² {self.b_sign} {str(self.b).replace('-', '') if '-' in str(self.b) else self.b}{self._variable_name} {self.c_sign} {str(self.c).replace('-', '') if '-' in str(self.c) else self.c} = 0
D = b² - 4ac = {self.b ** 2}² - 4 * ({self.a}) * ({self.c}) = {self.discriminant}
{self._variable_name}₁ = (-b + √discriminant)/(2 * a) = (-({self.b}) + √{self.discriminant})/(2 * {self.a}) = {self.root1}
{self._variable_name}₂ = (-b - √discriminant)/(2 * a) = (-({self.b}) - √{self.discriminant})/(2 * {self.a}) = {self.root2}"""
            else:
                self.return_string = f"""{equation}
{self.a_left_side}{self._variable_name}² {self.b_left_sign} {str(self.b_left_side).replace('-', '') if '-' in str(self.b_left_side) else self.b}{self._variable_name} {self.c_left_sign} {str(self.c_left_side).replace('-', '') if '-' in str(self.c_left_side) else self.c_left_side} = 0
{self.a}{self._variable_name}² {self.b_sign} {str(self.b).replace('-', '') if '-' in str(self.b) else self.b}{self._variable_name} {self.c_sign} {str(self.c).replace('-', '') if '-' in str(self.c) else self.c} = 0
D = b² - 4ac = {self.b ** 2}² - 4 * ({self.a}) * ({self.c}) = {self.discriminant}
{self._variable_name}₁ = (-b + √discriminant)/(2 * a) = (-({self.b}) + √{self.discriminant})/(2 * {self.a}) = {self.root1}
{self._variable_name}₂ = (-b - √discriminant)/(2 * a) = (-({self.b}) - √{self.discriminant})/(2 * {self.a}) = {self.root2}"""
